<script lang="ts">
	...
	let baseBranchStatus: { name: string; status: string; color: string; class: string } | null =
		null;
...
<div class="container mx-auto max-w-4xl p-4">
	...
	{#if prHeader && !error}
		...
		<div class="card mb-6 bg-base-100 shadow-xl">
			<div class="card-body">
				<h2 class="card-title flex items-start justify-between text-2xl">
					<a
						href="https://github.com/nixos/nixpkgs/pull/{prNumber}"
						target="_blank"
						class="link break-all text-primary link-hover"
					>
						{prHeader.title}
					</a>
				...
				<div class="collapse-arrow collapse mb-4 rounded-box bg-base-200">
					<input type="checkbox" />
					<div class="text-md collapse-title font-medium">PR Description</div>
				...
				<div class="mb-4 flex flex-wrap items-center gap-2">
					<span
						class="badge badge-lg {prHeader.closed && !prHeader.merged
							? 'badge-error'
							: prHeader.merged
								? 'badge-success'
								: 'badge-neutral'}"
				...
				{#if ciStatuses.length > 0}
					<div class="collapse-arrow collapse mb-4 rounded-box bg-base-200">
						<input type="checkbox" />
						<div class="text-md collapse-title font-medium">CI Status</div>
						<div class="collapse-content">
								...
								{#each ciStatuses as status}
									<div class="flex items-center gap-2">
										...
										<a
											href={status.url}
											target="_blank"
											class="flex-1 link truncate link-hover"
											title={status.description || status.state}
										>
											{status.name}
										</a>
										<span class="text-xs text-base-content/60 uppercase"
											>{status.state.replace('_', ' ')}</span
										>
									</div>
								{/each}
							</div>
					...
					<div>
						...
						<p class="mt-1">
							...
							<span class="font-mono text-xs"
								>{prHeader.merge_commit_sha
									? prHeader.merge_commit_sha.substring(0, 7)
									: 'N/A'}</span
							>
						</p>
					</div>
		...
		<h3 class="mb-4 text-xl font-bold">Branch Status</h3>
		<div class="grid gap-4 md:grid-cols-2">
			{#if baseBranchStatus}
				<div
					class="card border-l-4 bg-base-100 shadow-xl {baseBranchStatus.color === 'success'
						? 'border-success'
						: 'border-error'}"
				>
					<div class="card-body">
						...
						<div
							class="badge {baseBranchStatus.color === 'success'
								? 'badge-success'
								: 'badge-error'} gap-2"
						>
			...
			{:else}
				{#each branches as branch}
					<div
						class="card border-l-4 bg-base-100 shadow-xl {branchesStatus[branch]?.color ===
						'success'
							? 'border-success'
							: branchesStatus[branch]?.color === 'error'
								? 'border-error'
								: 'border-neutral'}"
					>
						<div class="card-body flex-row items-center justify-between">
							...
							{:else}
								<div
									class="badge {branchesStatus[branch]?.color === 'success'
										? 'badge-success'
										: branchesStatus[branch]?.color === 'error'
											? 'badge-error'
											: 'badge-neutral'} badge-lg"
								>
	...
	{:else if loading && !error}
		<div class="flex justify-center p-10">
			<span class="loading loading-lg loading-infinity text-primary"></span>
		</div>
	{/if}
</div>
